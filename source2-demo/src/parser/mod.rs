//! Parser module for Source 2 demo files.
//!
//! This module provides the main [`Parser`] type and related functionality
//! for parsing Source 2 engine demo files.
//!
//! # Overview
//!
//! The parser works by processing demo commands sequentially and calling
//! registered observers when events occur. Observers can filter which events
//! they're interested in using the [`Interests`] bitflags.
//!
//! # Example
//!
//! ```no_run
...
//!
//! // Register observers
//! let observer = parser.register_observer::<MyObserver>();
//!
//! // Run the parser
//! parser.run_to_end()?;
//! # Ok(())
...
impl<'a, R> Parser<'a, R>
where
    R: BitsReader + MessageReader + ReplayInfoReader,
{
